...
const DeleteExpenseModal = () => {
  ...
  const { showDeleteModal, deletingExpense } = useSelector((state) => state.ui);
  const { expenses, filters } = useSelector((state) => state.expenses);
  const [isDeleting, setIsDeleting] = useState(false);

  const expenseToDelete = deletingExpense
    ? expenses.find((expense) => expense.id === deletingExpense)
    : null;
  ...
  const handleDelete = async () => {
    if (deletingExpense && !isDeleting) {
      setIsDeleting(true);
      try {
        await dispatch(deleteExpense(deletingExpense));
